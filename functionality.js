...
window.addEventListener(
  ...
  (event) => {
    if (
      ...
      event.target.classList.contains("input-screen") ||
      event.target.classList.contains("Calc-type") ||
      event.target.parentElement.classList.contains("nav-bar") ||
      event.target.classList.contains("calculator")
    ) {
      menuDisableClickHandler(event);
    } else if (
      event.target.parentElement.classList.contains("action-buttons-grid") &&
...
window.addEventListener("keydown", (event) => {
  ...
  if (calculatorType.textContent === "Calculator") {
    if (currentKey.charCodeAt(0) >= 48 && currentKey.charCodeAt(0) <= 57) {
      console.log("Number:", event);
      updateInput(event.key, calcInputs);
    }

    switch (currentKey) {
      case "+":
        updateInput(currentKey, calcInputs);
        break;
      case "-":
        updateInput(currentKey, calcInputs);
        break;
...
function submitTipForm(event) {
  ...
  let tipTotal = tipFormCalc(
    formChildren[1].value,
    formChildren[3].value,
    formChildren[5].value,
  );
...
function submitDiscountForm(event) {
  ...
  let discountTotal = discountFormCalc(
    formChildren[1].value,
    formChildren[3].value,
    formChildren[5].value,
  );
  ...
  if (!darkMode) {
    ...
    switch (calculatorType.textContent) {
      ...
      case "Discount Calculator":
        ...
        break;
      case "Calculator":
        for (const input of document.querySelector(".action-buttons-grid")
          .children) {
    ...
    document.body
      ...
      .classList.remove("calculator-dark");
...
function tipCalculatorClickHandler(event) {
  ...
  document
    .querySelector(".action-buttons-grid")
    .classList.add("action-buttons-grid-form");
  ...
  document
    .querySelector(".action-buttons-grid")
    .replaceChildren(tipFormContainer);
...
function discountCalculatorClickHandler(event) {
  ...
  document
    .querySelector(".action-buttons-grid")
    .replaceChildren(discountFormContainer);
...
function calculatorClickHandler(event) {
  ...
  document
    .querySelector(".action-buttons-grid")
    .removeChild(document.querySelector(".calc-form-grid"));
  ...
  document
    .querySelector(".action-buttons-grid")
    .classList.remove("action-buttons-grid-form");
...
function inputParser(currentScreenText) {
  ...
  while (stack.length > 1) {
    ...
    if (stack.includes("(")) {
      ...
      console.log(
        `The placeholder index in stack to insert solution is: ${innerOpenParenthsIndex} \n`,
      );
      ...
      while (expression.length > 1) {
        expression.map((char, index) => {
          switch (char) {
            case "x":
              console.log(
                `${char} , found  at ${index}, adding to operation stack`,
              );
        ...
        orderOfOperations.sort((a, b, c, d) => {
        ...
        console.log(
          `Time to start solving the expression! \n Current Expression is: ${expression}`,
        );
        ...
        console.log(
          `The spliced expression after solving first operation: ${expression} \n It's length is now ${expression.length} \n`,
        );
